...
for index, customer in df.iterrows():
    ...
    prompt = f"""
    아래와 같은 데이터를 가진 고객이 우리 회사의 구독 서비스를 해지하지 않도록 붙잡을만한 메시지를 생성해줘.
    
    - 고객님 나이: {age}세 (나이는 메시지에서 언급하지마)
    - 고객님 선호 카테고리: {preferred_category}
    - 고객님의 마지막 로그인 후 {days_since_last_login}일이 지남.

    고객님에게 다음과 같은 내용을 포함하는 메시지를 생성해줘:
    ...
    response = openai.ChatCompletion.create(
        ...
        messages=[
            {"role": "system", "content": "고객 이탈을 막기 위한 맞춤형 메시지를 생성해줘."},
